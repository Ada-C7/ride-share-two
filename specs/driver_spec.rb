...
describe "ALL_DRIVERS" do
  ...
  it "has the right length for each element" do
    all_drivers.each do |driver|
      driver[:driver_id].size.must_equal all_drivers.length

      driver[:vin].size.must_equal all_drivers.length

      driver[:name].size.must_equal all_drivers.length
    end
  end
